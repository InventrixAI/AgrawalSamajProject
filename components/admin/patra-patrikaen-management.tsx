...
export default function PatraPatrikaenManagement() {
  const [pdfs, setPdfs] = useState<PatraPatrikaenPdf[]>([])
  ...
  const fetchPdfs = async () => {
    try {
      const response = await fetch("/api/admin/patra-patrikaen")
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              <CardTitle>Patra Patrikaen Management</CardTitle>
              <CardDescription>Manage Patra Patrikaen PDF documents</CardDescription>
            </div>
            <Dialog open={addPdfDialogOpen} onOpenChange={setAddPdfDialogOpen}>
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  <DialogTitle>Upload New Patra Patrikaen PDF</DialogTitle>
                </DialogHeader>
        ...
        <CardContent>
          ...
          {loading ? (
            ...
            <p className="text-sm text-gray-500">No Patra Patrikaen PDFs uploaded yet. Use the "Add New PDF" button above.</p>
          ) : (
